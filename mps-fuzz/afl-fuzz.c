...
EXP_ST u8  virgin_bits[MAP_SIZE << 1],     /* Regions yet untouched by fuzzing */             // fxm
           virgin_tmout[MAP_SIZE << 1],    /* Bits we haven't seen in tmouts   */             // fxm
           virgin_crash[MAP_SIZE << 1];    /* Bits we haven't seen in crashes  */             // fxm
...
EXP_ST u32 queued_paths,              /* Total number of queued testcases */
          ...
          //  path_with_dif_edge_align;   /* fxm: path has no new cov but new edge align    */
...
static inline u8 has_new_bits(u8* virgin_map) {                                                 // fxm
    ...
    if (unlikely(*current) && unlikely(*current & *virgin)) {
      ...
      if (likely(ret < 2)) {
        ...
        if ((cur[0] && vir[0] == 0xff) || (cur[1] && vir[1] == 0xff) ||
            (cur[2] && vir[2] == 0xff) || (cur[3] && vir[3] == 0xff) ||
            (cur[4] && vir[4] == 0xff) || (cur[5] && vir[5] == 0xff) ||
            (cur[6] && vir[6] == 0xff) || (cur[7] && vir[7] == 0xff)) ret = 1.5;                   /*   fxm   */       
#else
...
EXP_ST void setup_shm(void) {
  ...
  memset(virgin_tmout, 255, MAP_SIZE << 1);
  memset(virgin_crash, 255, MAP_SIZE << 1);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  memset(trace_bits, 0, MAP_SIZE << 1);                                                // fxm
  MEM_BARRIER();
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  if (q->exec_cksum) {
    ...
    memcpy(first_trace, trace_bits, MAP_SIZE);                                    // fxm explain: the size of first_trace is half of trace_bits, so only
    hnb = has_new_bits(virgin_bits);                                              //              the first half part will be copied
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    cksum = hash32(trace_bits, MAP_SIZE, HASH_CONST);                      //   fxm explain: for a case, gain hash value for the path
  ...
  q->bitmap_size = count_bytes(trace_bits);                               //  fxm
  q->handicap    = handicap;
  q->cal_failed  = 0;

  total_bitmap_size += q->bitmap_size;                                    //  fxm
...
static void show_stats(void) {
  ...
  t_bytes = count_non_255_bytes(virgin_bits);                                    //  fxm:  this fun only calculate the first mapsiz of virgin_bits
...
#define SP20    SP10 SP10
  ...
  sprintf(tmp, "%s / %s", DI(queued_with_cov), DI(queued_paths));                              //  fxm
